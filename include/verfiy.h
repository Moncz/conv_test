                    ...
                    {                       
                        for(int khNum = 0; khNum < r; khNum++)
                        {
                            for(int kwNum = 0; kwNum < s; kwNum++)
                            {
                                ...
                                {
                                    // sum += (float)(pin[nNum*c*h*w + posh_ori*w*c + posw_ori*c + cNum] * pwei[kNum*r*s*c + khNum*s*c + kwNum*c +cNum]);
                                    sum += (float)(pin[nNum*c*h*w + cNum*(w*h)+ posh_ori*w + posw_ori] * pwei[kNum*r*s*c + cNum*r*s + khNum*s + kwNum]);
                                }
                            }                       
                        }
                    }
                    // pout[nNum*k*oh*ow + i*ow*k + j*k + kNum] = (_Float16)sum;
